...
# define KBD_MIN_AUTOFIRE_TIME   1000000
 ...
 static unsigned int   loc_last_keyboard_event_time = 0;
 ...
 static int keyboard_mo5_key      = 0;
 static int keyboard_mo5_pending  = 0;
...
int
psp_kbd_enter_keyboard()
{
  int mo5_key   = 0;
  int keyboard_sym = -1;
  gp2xCtrlData  c;

  c.TimeStamp = gp2xCtrlGetTimeStamp();

  if (keyboard_mo5_pending) 
  {
    if ((c.TimeStamp - loc_last_keyboard_event_time) > KBD_MIN_PENDING_TIME) {
      loc_last_keyboard_event_time = c.TimeStamp;
      keyboard_mo5_pending = 0;
      mo5_key_event(keyboard_mo5_key, 0);
    }
    return 0;
  }

  keyboard_sym = gp2xCtrlReadKeyboardSym();
  if (keyboard_sym > 0) {
    ...
    if (keyboard_sym == SDLK_LEFT) {
      mo5_key = MO5K_LEFT;
    } else
    if (keyboard_sym == SDLK_RIGHT) {
      mo5_key = MO5K_RIGHT;
    } else
    if (keyboard_sym == SDLK_UP) {
      mo5_key = MO5K_UP;
    } else
    if (keyboard_sym == SDLK_DOWN) {
      mo5_key = MO5K_DOWN;
    } else {
      mo5_key = mo5_get_key_from_ascii(keyboard_sym);
    }
    if (mo5_key != -1) {
      keyboard_mo5_key     = mo5_key;
      keyboard_mo5_pending = 1;
...
{
  int wake = 0;
  int reverse_analog = ! MO5.psp_joystick_cursor;
